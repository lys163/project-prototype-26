...
| Test | JUnit Platform, Web MVC/Security test support, 4개 test class와 28개 automated test |
...
현재 `src/test`에는 `SecurityConfig` characterization, `BookService`/`ReviewService` ownership, OAuth2 token-log 비노출을 검증하는 4개 test class와 28개 automated test가 있습니다. 현재 checkout의 focused test, 전체 Gradle test와 clean build가 PASS했습니다. GitHub Actions CI의 마지막 확인 결과는 P0-2 Reading Progress authorization 변경 `main` push PASS이며, 이번 Profile authorization 변경은 아직 원격 CI 실행 전입니다. 이 최소 안전망은 전체 API contract 또는 application runtime 검증을 의미하지 않습니다.
